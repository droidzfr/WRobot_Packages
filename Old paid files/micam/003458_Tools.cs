...
public static class Tools
{
	static Tools()
	{
		robotManager.Events.ProductEvents.OnProductStopped﻿ ﻿+= OnProdu﻿ctStopped;
		ControlAuthentication.ControlAuthenticationStart();
	}
		
	private static void OnProductStopped﻿(string str)
	{
		Logging.Write("[Micam's Tools] Tools stopped");
		ControlAuthentication.KillThread();
	}
	...
	public static bool ClassQuest = true;
	...
	public static void TakeShipMenethilToTheramore()
    {
		Tools.TakeShip(176231, (int)ContinentId.Azeroth, "Wetlands", (int)ContinentId.Kalimdor, "Dustwallow Marsh",
			new Vector3(-3905.225, -585.8085, 0.0, "None"), new Vector3(-3898.902, -601.1245, 5.367361, "None"),
			new Vector3(-4016.39, -4740.588, 0.0, "None"),
    ...
    public static void TakeShipTheramoreToMenethil()
    {
		...
		new Vector3(-4016.39, -4740.588, 0.0, "None"), new Vector3(-4007.245, -4728.329, 5.248651, "None"),
		new Vector3(-3905.225, -585.8085, 0.0, "None"),
		4.551151f, 3000, 1.108747f, 3000);
    }
	
	public static void TakeZeppelinUndercityToOrgrimmar()
    {
		Tools.TakeShip(164871, (int)ContinentId.Azeroth, "Tirisfal Glades", (int)ContinentId.Kalimdor, "Durotar",
			new Vector3(2062.376, 292.998, 114.973, "None"), new Vector3(2066.623, 289.5278, 97.03132, "None"),
			new Vector3(1318.107, -4658.047, 71.86043, "None"),
			1.324273f, 1200, 1.55611f, 1500);
    }

    public static void TakeZeppelinOrgrimmarToUndercity()
    {
		Tools.TakeShip(164871, (int)ContinentId.Kalimdor, "Durotar", (int)ContinentId.Azeroth, "Tirisfal Glades",
		new Vector3(1318.107, -4658.047, 71.86043, "None"), new Vector3(1320.241, -4654.042, 53.648, "None"),
		new Vector3(2062.376, 292.998, 114.973, "None"),
		3.196529f, 1200, 1.656385f, 1500);
    }
			...
			else if ((Usefuls.ContinentId == continentTo && Usefuls.MapZoneName == zoneTo) || (continentFrom != continentTo && Usefuls.ContinentId == continentTo))
			{
				if (ObjectManager.Me.InTransport)
				{
					...
					if (transport != null && transport.Position.DistanceTo(toTransportWaitPosition) < 10)
					{
						...
						//wManager.Plugin.PluginsManager.LoadAllP﻿lugins();
						return;
					}
				}
			}
    ...
    public static void TakeTaxi(string fromTown, string toTown, int continent)
    {
		...
		Logging.Write("[Micam's Tools] Taking taxi from [" + fromTown + " (npc id = " + flyMasterFrom + ")] to [" + toTown + " (npc id = " + flyMasterTo + ")]");
		...
		while(ObjectManager.Me.Position.DistanceTo2D(flyMasterPosFrom) < 300)
		{
			if (!ObjectManager.Me.IsOnTaxi) 
			{
				if (wManager.Wow.Bot.Tasks.GoToTask.ToPositionAndIntecractWithNpc(flyMasterPosFrom, flyMasterFrom))
				{
					...
					if (taxiButtonId == -1)
					{
						Logging.Write("[Micam's Tools] Can't take taxi, destination not learned yet : " + toTown);
						return;
					}
	...
	public static void SpeakWithNpc(int idNpc, Vector3 npcPos)
    {
		bool flag = false;
		while (!flag)
		{
	...
	public static void TurnInQuestBugged(int idNpc, Vector3 npcPos)
    {
		bool flag = false;
		while (!flag)
		{
    ...
    {
		Logging.Write("[Micam's Tools] Learning Spells From Npc : Starting.");
		bool learned = false;
		while (!learned)
		{
			if (wManager.Wow.Bot.Tasks.GoToTask.ToPositionAndIntecractWithNpc(npcPos, idNpc))
		...
		Logging.Write("[Micam's Tools] Learning Spells From Npc : Finished.");
    }
    ...
    public static bool Craft(int Profession, string spellName)
    {
		if ((int)wManager.Wow.Enums.SkillLine.FirstAid == Profession)
		{
			Logging.Write("[Micam's Tools] First Aid Crafting : " + spellName);
	...
	*/
	...
	{
		Logging.Write("[Micam's Tools] TurnInCustom : Name = " + currentQuestClass.Name + " | QuestId = " + currentQuestClass.QuestId.First());
		...
		Logging.Write("[Micam's Tools] TurnInCustom : logId = " + logId + " | numQuestLogChoices = " + numQuestLogChoices);
		...
		for (int i = 1; i <= numQuestLogChoices; i++)
        {
			...
			Logging.Write("[Micam's Tools] TurnInCustom : item " + i + " = " + link + " | sType = " + sType + " | sSubType = " + sSubType);
		...
		Logging.Write("[Micam's Tools] TurnInCustom : item selected = " + currentQuestClass.GossipOptionItem);		
	...
	public static void AuthSystem(string productId)
	{
		wManager.Wow.Helpers.Conditions.ForceIgnoreIsAttacked = false;
		
		Tools.productId = productId;
		
		CustomQuesterAuthSettings.Load();
		
        if (CustomQuesterAuthSettings.CurrentSetting.TransactionId == null)
        {
            CustomQuesterAuthSettings.CurrentSetting.ToForm();
        }

        CustomQuesterAuthSettings.CurrentSetting.Save();
		if (!String.IsNullOrEmpty(CustomQuesterAuthSettings.CurrentSetting.TransactionId))
		{
			new Authentication(CustomQuesterAuthSettings.CurrentSetting.TransactionId, Tools.productId);
			ControlAuthentication.SetControl(5);
		}
		
		Tools.ClassQuest = CustomQuesterAuthSettings.CurrentSetting.ClassQuest;
	}
	...
	public class Authentication
	{
		...
		public Authentication(string orderId, string productId)
		{
			...
			authUrl = "http://51.75.194.139:8080/authenticate?orderId=" + orderId + "&productId=" + productId + "&wRobotAuthKey=" + wRobotAuthKey;
			_isRunning = true;
			...
			while (Products.IsStarted && _isRunning)
			{
				try
				{
					...
					if (!bool.Parse(result) || (content != null && content.StatusCode != HttpStatusCode.OK) || timedOut)
					{
						...
						Logging.Write("[Micam's Tools] Successfully authenticated");
	...
	public static class ControlAuthentication
	{
		...
		public static void ControlAuthenticationStart()
		{
			if (thread != null)
			{
				thread.Abort();
				Logging.Write("[Micam's Tools] Security thread stopped to be relaunched");
			}
			...
			thread = new Thread(() =>
			{
				...
				{
					Products.ProductStop();
					MessageBox.Show("Stop trying to get past the first step you're not that smart");					
				}
			});
			Logging.Write("[Micam's Tools] Security thread launched");
			thread.Start();
		}
		...
		public static void KillThread()
		{
			...
			{
				thread.Abort();
				Logging.Write("[Micam's Tools] Security thread stopped");
			}
		}
	}

	[Serializable]
	public class CustomQuesterAuthSettings : robotManager.Helpful.Settings
	{
		[Setting]
		[Category("__IMPORTANT__")]
		[DisplayName("Rocketr Transaction ID")]
		...
		[Setting]
		[Category("__IMPORTANT__")]
		[DisplayName("Class Quest")]
		[Description("Enable/disable class quest")]
		public bool ClassQuest { get; set; }

		public CustomQuesterAuthSettings()
		{
			TransactionId = null;
			ClassQuest = true;
		}
		
		public static CustomQuesterAuthSettings CurrentSetting { get; set; }

		public bool Save()
		{
			try
			{
				return Save(AdviserFilePathAndName("MicamQuesterAuthSettings", ObjectManager.Me.Name + "." + Usefuls.RealmName + "." + Tools.productId));
			}
			catch (Exception e)
			{
				robotManager.Helpful.Logging.Write("MicamQuesterAuthSettings > Save(): " + e);
				return false;
			}
		}
		...
		public static bool Load()
		{
			try
			{
				if (File.Exists(AdviserFilePathAndName("MicamQuesterAuthSettings", ObjectManager.Me.Name + "." + Usefuls.RealmName + "." + Tools.productId)))
				{
					CurrentSetting = Load<CustomQuesterAuthSettings>(AdviserFilePathAndName("MicamQuesterAuthSettings", ObjectManager.Me.Name + "." + Usefuls.RealmName + "." + Tools.productId));
					return true;
				}
				CurrentSetting = new CustomQuesterAuthSettings();
			}
			catch (Exception e)
			{
				robotManager.Helpful.Logging.Write("MicamQuesterAuthSettings > Load(): " + e);
			}
			return false;
		}
	}
