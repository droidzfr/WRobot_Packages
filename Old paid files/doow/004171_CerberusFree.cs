...
public class Main : IPlugin
{
    ...
    private int DefaultMinSlots;
    private int AmmoBagId;
    ...
    public void Initialize()
    {
        ...
        {
            AmmoBagId = GetAmmoContainerSlotId();
            DefaultMinSlots = wManagerSetting.CurrentSetting.MinFreeBagSlotsToGoToTown;
        } 
    }
    ...
    public void Dispose()
    {
        ...
        wManagerSetting.CurrentSetting.MinFreeBagSlotsToGoToTown = DefaultMinSlots;
        _isLaunched = false;
    ...
    private void WatchForLuaEvents()
    {
        ...
        EventsLuaWithArgs.OnEventsLuaWithArgs += (LuaEventsId id, List<string> args) =>
        {
            ...
            {
                if (CerberusSettings.CurrentSetting.StartAreaHelper && ObjectManager.Me.Level < 11)
                {
                    Logging.Write(String.Format("{0: [Cerberus Bag Watcher] Your Bag Value is #### Gold ## Silver and ## Copper}", BagValue(), Logging.LogType.Normal));
                    Logging.Write(String.Format("{0: [Cerberus Bag Watcher] Your Money Value is #### Gold ## Silver and ## Copper}", CurrentMoney(), Logging.LogType.Normal));
                    Logging.Write(String.Format("{0: [Cerberus Bag Watcher] Your Total Value is #### Gold ## Silver and ## Copper}", TotalMoney(), Logging.LogType.Normal));                   
                }
            }
    ...
    public static void BotSettings()
    {
        ...
        try
        {
            ...
            {
                bool autolootenabled = Lua.LuaDoString<bool>("return GetAutoLootDefault()");
                if (autolootenabled == false)
                    Lua.RunMacroText﻿("/script SetAutoLootDefault(true);");
            }
    ...
    public void StartAreaHelper()
    {
        ...
        else if (ObjectManager.Me.Level < 8)
        {
            ...
            {
                Logging.Write("[Cerberus Bag Watcher]:" + " Your Total Value is " + TotalMoney() + " We are now selling to have money for Class Trainer lvl 8");
                ToTown.ForceToTown = true;
            }
        }
    }
    ...
    public void RacialInitiator()
    {
        ...
        else if (RacialCD() == 0 && RacialSettings())
        {
            RacialSkillManager();
        }
    }
    ...
    {
        var Hostile = ObjectManager.GetNumberAttackPlayer();
        var Race = ObjectManager.Me.PlayerRaceString;
        ...
        else if (Race == "Human" && Information.ForWowBuild <= 8606)
        {
            return false;
        }
        else if (Race == "Human" && Information.ForWowBuild > 8606)
        ...
        {
            if (ObjectManager.Me.InCombat && (ObjectManager.Target.HealthPercent >= 80 || Hostile > 1))
                return true;
        }
        else if (Race == "Troll")
        {
        ...
        {
            if (ObjectManager.Target.IsCast && (ObjectManager.Me.InCombat && Distance <= 8) || (Hostile >= 2))
                return true;
        }
        return false;
    }
    ...
    {
        wManagerSetting.CurrentSetting.MinFreeBagSlotsToGoToTown = DefaultMinSlots + Bag.GetContainerNumFreeSlotsByBagID(AmmoBagId);
    }
    ...
    public class CerberusSettings : Settings
    {
        ...
        [Description("Runecloth and Netherweave Cloth")]
